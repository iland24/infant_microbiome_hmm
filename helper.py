...
def dmm(otu_df, csv_file_path, dmm_output_name, path_to_dmm_output_filename, n_clusters = 6,):
    ...
    *This fuction is made to checks if the output df 
    ...
    while final_ok == False: 
        ...
        df_percent = cluster_assignments_df["cluster"].value_counts()
        df_percent = (df_percent/df_percent.sum())*100
        df_percent.sort_index(inplace=True)
        df_percent.plot(kind="bar",figsize=(6,6),xlabel='states',ylabel='percentage')
...
def output_i_t_e_matrix(taxa_name,i_m,t_m,e_m, path):
    ...
    # save output as .npy pickle file
    np.save(path+'/initial_matrix.txt', np.exp(i_m))
    np.save(path+'/transition_matrix', np.exp(t_m))
    np.save(path+'/emission_matrix', np.exp(e_m))
...
def convert_otu_to_data_for_prediction(csv_data_filepath, pma_start_date=196):
    ...
    test_otu_df, taxa_names = choose_top_x_percent_of_taxa_with_highest_variance(test_otu_df, frac=0.0338)
